...
import { getProspectingCustomerById, updateProspectingCustomerById } from 'apiSdk/prospecting-customers';
...
function ProspectingCustomerEditPage() {
  const router = useRouter();
  const id = router.query.id as string;
  const { data, error, isLoading, mutate } = useSWR<ProspectingCustomerInterface>(
    () => (id ? `/prospecting-customers/${id}` : null),
    () => getProspectingCustomerById(id),
  );
  ...
  const handleSubmit = async (values: ProspectingCustomerInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await updateProspectingCustomerById(id, values);
      mutate(updated);
      resetForm();
