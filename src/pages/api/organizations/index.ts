...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  async function createOrganization() {
    ...
    const body = { ...req.body };
    if (body?.prospecting_customer?.length > 0) {
      const create_prospecting_customer = body.prospecting_customer;
      body.prospecting_customer = {
        create: create_prospecting_customer,
      };
    } else {
      delete body.prospecting_customer;
    }
